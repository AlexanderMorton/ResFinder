...
int systExists(string outDir, string region, string bkgd, string syst, string upTag, string dnTag){
  ...
  string outNameD = string(syst).append(dnTag);
  string outNameU = string(syst).append(upTag);
  ...
  if(myConfig._runFromWS){
	...
	for(uint mm=0; mm<myConfig._massList.size(); mm++){
	  ...
	  for(uint syst=0; syst<myConfig._shapeSystNames.size(); syst++){
	    ...
	    int retval = systExists(myConfig._outDir, myConfig._regionNames[rr], thisSigName, myConfig._shapeSystNames[syst],myConfig._systUpTag, myConfig._systDnTag);
	    if(retval==2){
	      vhAna.channel(myConfig._regionNames[rr]).sample(thisSigName).addVariation(myConfig._shapeSystNames[syst], myConfig._systUpTag, myConfig._systDnTag);
	    }
	    else if(retval==1){
	      vhAna.addOneSideVariation(myConfig._shapeSystNames[syst], myConfig._systUpTag, myConfig._systDnTag);
	    }
	  }
	...
	//Add shape systematics from TH1 format
	for(uint syst=0; syst<myConfig._shapeSystNames.size(); syst++){
	  int retval = systExists(myConfig._outDir, myConfig._regionNames[rr], myConfig._bkgdNames[bb], myConfig._shapeSystNames[syst], myConfig._systUpTag, myConfig._systDnTag); 
	  if(retval==2){
	    vhAna.channel(myConfig._regionNames[rr]).sample(myConfig._bkgdNames[bb]).addVariation(myConfig._shapeSystNames[syst], myConfig._systUpTag, myConfig._systDnTag); 
	  }
	  else if(retval==1){
	    vhAna.addOneSideVariation(myConfig._shapeSystNames[syst], myConfig._systUpTag, myConfig._systDnTag); 
	  }
    ...
    if(myConfig._dataScale!=1) vhAna.scaleData(myConfig._dataScale);
    if(myConfig._lumiScale!=1) vhAna.setLumiRescale(1.0, myConfig._lumiScale);
    if(myConfig._glSigScale!=1) vhAna.scaleSignal(myConfig._glSigScale, "All");

    //Set asimovData to TRUE for data = bkgd_sum
    vhAna.setDoObserved(!(myConfig._setAsimovData));
