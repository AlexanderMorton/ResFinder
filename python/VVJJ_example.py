...
# createDirectoryStructure('/afs/cern.ch/work/y/yygao/public/VVJJStatInput/results_CamKt12SplitFilteredMu100SmallR30YCut4/syst_100', releaseDir, analysis, inputListTag, outputWSTag)
  ...
  # set stat error threshold
  vvjj.channel('SR').setStatErrorThreshold(0.05) # means that errors < 5% will be ignored
  ...
  for mass in masses:
    ...
    # add MC stat uncertainty
    vvjj.channel('SR').sample(thisSig).setUseStatError(True)
  ...
  if not doExternalPdf:
    ...
    import os
    #os.system('cp /afs/cern.ch/user/v/vippolit/public/RF/Background.* .') # uncomment only if the second does not work
    os.system('cp /afs/cern.ch/user/v/vippolit/public/enrique/Background.* .') # auto binning
