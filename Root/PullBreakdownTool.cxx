...
RF::PullBreakdownTool::PullBreakdownTool(TString name) 
{
   init();
}

RF::PullBreakdownTool::~PullBreakdownTool()
{
}



void RF::PullBreakdownTool::init()
{

}

//RF::StatisticalResults RF::PullBreakdownTool::run(RooWorkspace *ws)
//{
//  RF::StatisticalResults result;
//  return result;
//}

void RF::PullBreakdownTool::setMass(Double_t mass) {
  m_mass = mass; 
}

void RF::PullBreakdownTool::run(RooWorkspace *ws,
 ...
 std::cout << "DEBUG : Start from PullBreakdownTool.cxx" << std::endl;
  ...
  folder = folder + "_" + Form("%.0f", m_mass) ; 
   ...
   LOG::ReportingLevel() = LOG::FromString(config->loglevel);
    ...
    system(("mkdir -vp root-files/" + config->folder + "_breakdown_" + technique).c_str());
    system(("rm root-files/" + config->folder + "_breakdown_" + technique + "/*").c_str()); // clean up
  ...
  std::cout << "DEBUG : End from PullBreakdownTool.cxx" << std::endl;
  return;
}
...
void RF::PullBreakdownTool::setParams(RooArgSet* nuis, list<string> tmp_list, string technique, RooFitResult* fitresult, RooRealVar* poi, double corrCutoff) {
    RooRealVar* var;
    TIterator* nitr = nuis->createIterator();
...
list<string> RF::PullBreakdownTool::addParams(settings* config, string catecory2eval) {
            ...
            while (systematic != 0) {
                ...
                while (attr_syst != 0) {
                    ...
                    if (breakdown) {
                        ...
                        //runBreakdown(config->inFileName, config->folder,        	     config->wsName, config->modelConfigName, config->dataName, config->poiName, "config/tmp_"+systName+".xml", config->technique, systName, config->precision, config->corrCutoff,      config->loglevel);
			run(config->ws, 
			    config->modelConfigName, 
			    config->dataName, 
			    config->poiName, 
			    "config/tmp_"+systName+".xml", 
			    config->technique, systName, config->precision, config->corrCutoff,      config->loglevel
			    ); // by Haifeng
...
void  RF::PullBreakdownTool::writeTmpXML (string systName, settings* config) {
